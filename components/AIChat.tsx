...
const AIChat: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'model', text: '¡Hola! Soy tu asistente virtual UBA. ¿Consultas sobre carreras o ingreso al CBC? 🎓' }
  ]);
  ...
  return (
    <div className="fixed bottom-4 right-4 md:bottom-6 md:right-6 z-50 flex flex-col items-end pointer-events-auto">
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="mb-4 w-[90vw] md:w-96 bg-[#0f172a]/95 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden shadow-2xl shadow-sky-500/10"
          >
            {/* Header */}
            <div className="bg-gradient-to-r from-slate-900 to-slate-800 p-4 flex justify-between items-center border-b border-white/10">
              <div className="flex items-center gap-2">
                <GraduationCap className="w-5 h-5 text-[#fbbf24] animate-pulse" />
                <h3 className="font-heading font-bold text-white tracking-wider">UBA CONSULTAS</h3>
              </div>
            ...
            <div className="p-3 border-t border-white/10 bg-black/40">
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="Preguntá sobre carreras..."
                  className="flex-1 bg-transparent text-white placeholder-white/30 text-sm focus:outline-none"
                />
                <button
                  onClick={handleSend}
                  disabled={isLoading || !input.trim()}
                  className="bg-[#fbbf24] p-2 rounded-lg hover:bg-yellow-400 transition-colors disabled:opacity-50"
                  data-hover="true"
                >
      ...
      <motion.button
        ...
        className="w-12 h-12 md:w-14 md:h-14 rounded-full bg-gradient-to-tr from-[#fbbf24] to-[#0ea5e9] flex items-center justify-center shadow-lg shadow-sky-500/20 border border-white/20 z-50 group"
        data-hover="true"
      >
        {isOpen ? (
